...
API.prototype = {
    ...
    doRequest: function(method, path, params, callback)
    {
            ...
            res.on('end', function() {
                ...
                if (encoding == 'gzip')
                {
                    zlib.gunzip(responseBody, function(err, decoded) {
                        ...
                        {
                            processResponseJson(decoded.toString(), res.statusCode);
                        }
                    });
                }
                else
                {
                    processResponseJson(responseBody.toString(), res.statusCode);
                }
            });
        });
        ...
        var processResponseJson = function(responseJson, statusCode) {
            var responseData;
            try
            {
            ...
            {
                callback(new Error("Unexpected response from Telerivet API (HTTP " + statusCode + "): " + responseJson), null);
                return;
            }
